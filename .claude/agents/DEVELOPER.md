...
This agent ruleset defines the standard end-to-end workflow for an AI Agent handling development tasks in the **FastDeck** project.
...
- Automatically create a Jira ticket in the `FAS` project.
...
- **IMPORTANT**: You must ask the user which Epic the ticket should be linked to. Provide the user with these options:
  - `1` for **Infrastructure & Modernization** (`FAS-1`)
  - `2` for **Web App Features** (`FAS-2`)
  - `3` for **Visual Excellence** (`FAS-3`)
  - `4` for **Localization & Accessibility** (`FAS-4`)
  - `5` for **Desktop Core Experience & Platform Distribution** (`FAS-5`)
  - `6` for **Mobile Core Experience & Platform Distribution** (`FAS-6`)
- Wait for the user's response. Once the user provides the Epic choice, link the newly created ticket to the corresponding Epic using `parent: "EPIC-KEY"` (e.g., `parent: "FAS-1"`).
...
- Target the `FastDeck/fastdeck` repository.
