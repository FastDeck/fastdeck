...
This ruleset defines the standard patterns for creating and managing Pull Requests in this repository. **All Pull Requests MUST be created in the FastDeck/fastdeck repository.**
...
> [!IMPORTANT]
> Every Pull Request created MUST use `owner: "FastDeck"` and `repo: "fastdeck"`.
...
**Example:**
`amitraikwar/FAS-20/update-jira-skills`

## 4. GitHub MCP Server Integration

When creating a PR using the `github-mcp-server` tool, ensure:

1. The `owner` and `repo` are set to `FastDeck` and `fastdeck`.
